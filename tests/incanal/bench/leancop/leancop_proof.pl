:- module(leancop_proof, [leancop_proof/2, set_proof_layout/1], [datafacts]).
...
leancop_proof(Mat,Proof) :-
    proof_layout(compact) -> leancop_compact_proof(Proof) ;
    proof_layout(connect) -> leancop_connect_proof(Mat,Proof) ;
