:- module(slots, [conc/3, i_sentence/2], [assertions]).
...
i_np_head0(np_head(quant(Op0,N),Adjs,Noun),Type-X,Type-X,void,'`'(P),Pred,Pred,[]) :-
...
i_adj(sup(Op0,adj(Adj)),Type-X,Type-V,_,aggr(F,V,[Y,X],Head,'`'('&'(P,Pred))),Head,'`'(true),Pred) :-
   sign(Adj,Sign),
   inverse(Op0,Sign,Op),
   i_sup_op(Op,F),
