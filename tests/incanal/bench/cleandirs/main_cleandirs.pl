:- module(main_cleandirs, [main/1], [assertions, datafacts]).
...
:- use_module(library(iso_char), [get_char/1]).
:- use_module(library(format), [format/2]).
:- use_module(library(aggregates), [setof/3, (^)/2]).

:- use_module(engine(runtime_control), [prompt/2]).

:- use_module(a_my_system, [modif_time/2, datime/9,
        working_directory/2, system/1, delete_file/1, cd/1,
        directory_files/2, file_exists/1, file_property/2]).

:- use_module(generate_db, [generates/2, can_generate_file/2]).
...
main(Arg):- 
	    ...
	    cd(Directory),
%	    catch(
               recurse_all_entries(Files, action(Action, Backups))%,
	%    Ball, exit(Ball))
        ;
            usage_message
        ).
...
check_right_use(Directory, Action, Backups):-
        ...
        ( member(Action, Second) ->
             true
        ;
            format("Second argument not valid: must be one of ~w~n", [Second]),
	    Fail = yes
        ),
        ( member(Backups, Third) ->
             true
        ; 
            format("Third argument not valid: must be one of ~w~n", [Third]),
	    Fail = yes
        ),
        Fail = no.
...
which can be generated from other files:~n", []),
	setof(X, Y^((generates(X, Y) ; generates(Y, X)), X \== ''), 
              ListOfSuff),
