...
:- use_module(library(stream_utils)). %[write_string/1, get_line/1]).
...
% :- include(library(pillow/icon_address)).

:- comment(icon_address(Img, IAddress), "The PiLLoW image @var{Img} has
           URL @var{IAddress}.").

:- pred icon_address(?atm,?atm).
icon_address(Img, IAddress):-
        icon_img(Img,ImgSrc),
        atom_concat('/images/',ImgSrc,IAddress).
...
:- pred http_lines(Lines, String, Tail)
...
:- pred form_request_method(Method) => atm
...
%% :- pred url_query(+value_dict,-string).
...
%% :- pred url_query_amp(+value_dict,-string).
...
%% :- pred url_query_values(+value_dict,-string).
%% :- pred url_query_values(-value_dict,+string).
